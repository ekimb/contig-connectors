...
use super::mers;
use super::Params;
use crate::mers::{seq_to_kmers_nthash, Match, Mer};
...
fn read_lines_lz4<P>(
    filename: P,
) -> io::Result<io::Lines<BufReadDecompressor<'static, BufReader<File>>>>
where
    P: AsRef<Path>,
{
    let file = File::open(filename)?;
    Ok(BufReadDecompressor::new(BufReader::new(file))
        .unwrap()
        .lines())
}
...
pub fn index(
    ...
    let mut r_c = 4;
    let mut clusters: Arc<DashMap<Vec<(usize, u64)>, Vec<Vec<(usize, u64)>>>> =
        Arc::new(DashMap::new());
    ...
    let create_sequences_file = |thread_id: ThreadIdType| -> SeqFileType {
        ...
        seq_write(
            &mut sequences_file,
            "# [node name]\t[list of minimizers]\t[origin]\n".to_string(),
        );
        sequences_file
    };
    let add_sketch = |sk: &Vec<(usize, u64)>,
                      seq: Option<&str>,
                      origin: &str,
    ...
    let index_read_aux_mer = |seq_str: &[u8], seq_id: &str| -> Option<usize> {
        ...
        add_sketch(
            &sk,
            Some(std::str::from_utf8(seq_str).unwrap()),
            &seq_id,
            &mut sequences_file,
            thread_id,
        );
    ...
    let mut main_thread_mer = |found: &mut Option<usize>| {
        // runs in main thread
        return None::<usize>;
    };
    ...
    if fasta_reads {
        ...
        read_process_fasta_records(
            ...
            threads as u32,
            queue_len,
            index_read_fasta_mer,
            |record, found| main_thread_mer(found),
        );
    } else {
    ...
    let query_read_aux_mer = |seq_str: &[u8], seq_id: &str| -> Option<usize> {
        ...
        let sk = seq_to_kmers_nthash(seq_str, seq_id, params);
        if ids.get(&sk).is_some() {
            ids.get_mut(&sk).unwrap().push(seq_id.to_string());
        } else {
            ids.insert(sk.to_vec(), vec![seq_id.to_string()]);
        }
        if sk.is_empty() {
            return None;
        }
