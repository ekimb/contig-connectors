...
fn main() {
    ...
    match Msmash::from_args() {
        Msmash::Index { directory, k, l, density, threads, prefix } => {
            ...
            let queue_len = 200;
            let mut all_mers = Vec::<(Vec<Vec<u8>>, String)>::new();
            let mut mers_count = 0;
            let index_path = format!("{}{}", output_prefix.to_str().unwrap(), ".ms");
        ...
        Msmash::Query { index, query, k, l, density, threads, prefix } => {
            ...
            let res_path = format!("{}{}", output_prefix.to_str().unwrap(), ".hit");
