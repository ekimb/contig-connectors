...
pub fn insert(r_c: usize, sk: &Vec<(usize, u64)>, clusters: &DashMap<Vec<(usize, u64)>, Vec<Vec<(usize, u64)>>>) {
    if clusters.len() == 0 {clusters.insert(sk.to_vec(), vec![]); return;}
    else {
        ...
        for c in clusters.iter() {
            ...
            if dist < min_dist {min_dist = dist; min_center = center.to_vec();}
        }
        if min_dist == r_c {clusters.insert(sk.to_vec(), vec![]);}
        else if clusters.get_mut(&min_center).is_some() {clusters.get_mut(&min_center).unwrap().push(sk.to_vec());}
    }
}

pub fn hamming(a: &Vec<(usize, u64)>, b: &Vec<(usize, u64)>, r_c: usize) -> usize {
    let mut min_dist = r_c;
    if a == b {return min_dist;}
    let la = a.len();
    let lb = b.len();
    let ml = match la > lb {true => la, false => lb};
    if ml == la {
        let w = lb;
...
pub fn levenshtein(a: &Vec<(usize, u64)>, b: &Vec<(usize, u64)>, r_c: usize) -> usize {
    ...
    for (ib, tupb) in b.iter().enumerate() {
        ...
        for (ia, tupa) in a.iter().enumerate() {
            ...
            da = cache[ia];
            dist = if da > dist {
                if db > dist {dist + 1} else {db}
            } else if db > da {da + 1} else {db};
            if dist >= r_c {return r_c;}
            cache[ia] = dist;
        }
    }
    dist
        ...
        if hamming(center, sk, r_c) < r_c {
            for mem in membs.iter() {
                if hamming(mem, sk, r_c) == 0 {
                    let ml = match mem.len() > sk.len() { true => (mem.to_vec(), sk.to_vec()), false => (sk.to_vec(), mem.to_vec())};
                    res.push(ml.clone());
                }
            }
        }
